...
class Base(ABC):
    ...
    async def achat(self, system, history, gen_conf={}, **kwargs):
        loop = asyncio.get_running_loop()
        return await loop.run_in_executor(None, partial(self.chat, system, history, gen_conf, **kwargs))

    async def achat_streamly(self, system, history, gen_conf={}, **kwargs):
        loop = asyncio.get_running_loop()
        queue = asyncio.Queue()
        sentinel = object()
...
class LiteLLMBase(ABC):
    ...
    async def _aexceptions(self, e, attempt) -> str | None:
        logging.exception("OpenAI chat_with_tools")
        # Classify the error
        error_code = self._classify_error(e)
        if attempt == self.max_retries:
    ...
    async def achat(self, system, history, gen_conf={}, **kwargs):
        ...
        for attempt in range(self.max_retries + 1):
            try:
                response = await self._achat(history, gen_conf, **kwargs)
                return response
            except Exception as e:
                e = await self._aexceptions(e, attempt)
                if e:
                    return e, 0
        assert False, "Shouldn't be here."

    async def _achat(self, history, gen_conf, **kwargs):
        logging.info("[HISTORY]" + json.dumps(history, ensure_ascii=False, indent=2))
        if self.model_name.lower().find("qwq") >= 0:
            logging.info(f"[INFO] {self.model_name} detected as reasoning model, using _chat_streamly")
            ...
            final_ans = ""
            tol_token = 0
            async for delta, tol in self.achat_streamly(history, gen_conf, with_reasoning=False, **kwargs):
                if delta.startswith("<think>") or delta.endswith("</think>"):
                    continue
                final_ans += delta
                tol_token = tol
    ...
    async def achat_streamly(self, system, history, gen_conf: dict = {}, **kwargs):
        if system and history and history[0].get("role") != "system":
            history.insert(0, {"role": "system", "content": system})
        gen_conf = self._clean_conf(gen_conf)
        ans = ""
        total_tokens = 0
        try:
            async for delta_ans, tol in self._achat_streamly(history, gen_conf, **kwargs):
                yield delta_ans
                total_tokens += tol
        except openai.APIError as e:
            yield ans + "\n**ERROR**: " + str(e)
    ...
    async def _achat_streamly(self, history, gen_conf, **kwargs):
        logging.info("[HISTORY STREAMLY]" + json.dumps(history, ensure_ascii=False, indent=4))
        reasoning_start = False
    ...
    async def achat_with_tools(self, system: str, history: list, gen_conf: dict = {}):
        gen_conf = self._clean_conf(gen_conf)
        if system and history and history[0].get("role") != "system":
            history.insert(0, {"role": "system", "content": system})
        ...
        for attempt in range(self.max_retries + 1):
            ...
            try:
                ...
                response, token_count = await self._achat(history, gen_conf)
                ans += response
                tk_count += token_count
                return ans, tk_count
    ...
    async def achat_streamly_with_tools(self, system: str, history: list, gen_conf: dict = {}):
        gen_conf = self._clean_conf(gen_conf)
        tools = self.tools
        if system and history and history[0].get("role") != "system":
            history.insert(0, {"role": "system", "content": system})
